...
class FluxAndMonoControllerTest {
	...
	@Test
	void fluxStream() {
		Flux<Long> integerFlux = webTestClient.get().uri("/fluxstream").accept(MediaType.APPLICATION_STREAM_JSON)
				.exchange().expectStatus().isOk().returnResult(Long.class).getResponseBody();

		StepVerifier.create(integerFlux).expectNext(0l).expectNext(1l).expectNext(2l).thenCancel().verify();
	}
